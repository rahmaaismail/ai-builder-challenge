...
const STATE_COLORS: Record<AssetState, string> = {
  in_service: "text-emerald-700", stored: "text-amber-700", received: "text-blue-700",
  rma_pending: "text-orange-700", disposed: "text-gray-500", unreceived: "text-slate-500",
};
    ...
    <div className="space-y-5">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">{assets.length.toLocaleString()} asset{assets.length !== 1 ? "s" : ""}{activeFilter ? " (filtered)" : ""}</p>
        </div>
        <Link href="/manager/reconcile" className="text-sm font-medium text-blue-700 hover:underline whitespace-nowrap">Three-way reconciliation →</Link>
      </div>

      <div className="grid grid-cols-3 sm:grid-cols-6 gap-2">
        {STATE_ORDER.map((s) => (
          <Link key={s} href={`/manager?state=${s}`} className={`rounded-lg border bg-white px-3 py-2 text-center hover:bg-gray-50 transition-colors ${state === s ? "ring-2 ring-blue-500" : ""}`}>
      ...
      {paged.length === 0 ? (
        <div className="rounded-xl border bg-white p-12 text-center">
          <p className="text-gray-400">{activeFilter ? "No assets match this filter." : "No assets in the system yet."}</p>
          {activeFilter && <Link href="/manager" className="mt-2 inline-block text-sm text-blue-600 hover:underline">Clear filters</Link>}
        </div>
      ) : (
        <>
              ...
              <tbody className="divide-y divide-gray-100">
                {paged.map((asset) => (
                  <tr key={asset.asset_tag} className="hover:bg-gray-50">
                    <td className="px-4 py-3"><Link href={`/manager/assets/${asset.asset_tag}`} className="font-mono text-blue-700 hover:underline font-medium">{asset.asset_tag}</Link></td>
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-between text-sm">
              ...
              <div className="flex gap-2">
                {safePage > 1 && <Link href={`/manager?${new URLSearchParams({ ...(state ? { state } : {}), ...(site ? { site } : {}), ...(custodian ? { custodian } : {}), page: String(safePage - 1) })}`} className="rounded-lg border px-3 py-1.5 text-gray-700 hover:bg-gray-50">Previous</Link>}
                {safePage < totalPages && <Link href={`/manager?${new URLSearchParams({ ...(state ? { state } : {}), ...(site ? { site } : {}), ...(custodian ? { custodian } : {}), page: String(safePage + 1) })}`} className="rounded-lg border px-3 py-1.5 text-gray-700 hover:bg-gray-50">Next</Link>}
              </div>
            </div>
          )}
        </>
